---
log:
2026-08-15: Hardened compatibility state, settings, OAuth cache, logs, and JSONL history as private local data. CLI-managed directories are mode `0700`; private files and stable lock sidecars are mode `0600`, validated against symlinks, non-regular files, wrong ownership, and hard links. State writes now use same-directory atomic replacement under the existing read/write lock, malformed nonblank state raises without overwriting evidence, history validates session-derived filenames and serializes complete durable appends, and the idempotent legacy-history scrubber coordinates with live writers. Keep-alive history no longer stores response bodies, and session sync no longer converts an authentication failure into an empty successful listing.
...
- **Structured logging**: The daemon emits `keep_alive_started` (with `pid`, `endpoint`), one `keep_alive_error` per failed iteration (with `status_code`, `error_type`, truncated `error`, `iteration`, `consecutive_4xx`), and `keep_alive_stopped` (with `reason`, `iterations`, `duration_seconds`, optional sanitized `last_error`, optional `expected_endpoint`/`actual_endpoint`). Response bodies are deliberately excluded because control-plane errors can reflect credentials or private request data. All three are rendered specially by `colab log` so users get diagnostic context without parsing JSONL by hand.
- **Termination**:
...
- **Authentication**: Uses `google-auth-oauthlib` with the remote copy-paste OAuth flow documented in `04_automation_and_utility.md`; it does not bind a localhost callback server.
- **Global Flags**:
...
- **Token Storage**: Credentials are atomically persisted to private `~/.config/colab-cli/token.json` after the initial flow or refresh.
- Use `requests` for robust HTTP interactions and `pydantic` for schema validation.
- Authorized requests use bearer credentials in memory; headers, cookies, query credentials, and response bodies are excluded from persistent diagnostics.
...
- **Private paths**: The default `~/.config/colab-cli` and `history` directories are hardened to `0700`; state, settings, OAuth tokens, logs, JSONL history, and their stable lock files are hardened to `0600`. Leaf symlinks, non-regular files, wrong-owner files, and multiply-linked files fail closed. A caller-supplied `--config` parent is never chmodded, although the private file itself is still mode `0600`.
- **Crash-safe writes**: JSON stores are serialized to a mode-`0600` temporary file in the destination directory, fsynced, atomically replaced, and followed by a directory fsync. Blank or missing files retain default semantics; malformed nonblank JSON raises `LocalStateError` and is preserved for diagnosis instead of silently resetting state.
- **History safety**: Session names containing traversal separators, NULs, or dot-directory aliases are rejected before path construction. Appends hold the same stable per-session write lock used by the idempotent `scrub_legacy_history()` migration, preventing interleaving, partial JSON records, or migration-vs-append lost updates.
